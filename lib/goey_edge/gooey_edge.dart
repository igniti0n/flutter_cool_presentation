...
class GooeyEdge {
  ...
  GooeyEdge({int count = 10, this.side = Side.left}) {
    points = [];
  ...
  void applyTouchOffset([Offset? offset, Size? size]) {
    ...
    FractionalOffset o =
        FractionalOffset.fromOffsetAndSize(offset, size ?? Size.zero);
    if (side == Side.left) {
      touchOffset = o;
  ...
  void tick(Duration duration) {
    ...
    for (int i = 0; i < l; i++) {
      ...
      double ratio = max(0.0,
          1.0 - (pt.y - (touchOffset?.dy ?? 0.0)).abs() / maxTouchDistance);
      pt.velX += (touchOffset?.dx ?? 0 - pt.x) * touchTension * ratio * t;
      if (i > 0) {
